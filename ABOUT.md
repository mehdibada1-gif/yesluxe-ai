...
| `findRelevantFaq`                       | Finds the most relevant FAQ from a list based on a user's question, increasing its usage count if found.           | User question, list of FAQs, and Property ID.            | The ID of the best matching FAQ, or null.                            |
| `summarizeMarket`                       | A demo flow that uses a Genkit Tool to fetch a "stock price" as part of a market summary.                            | A text string (e.g., "Tell me about Google").            | A summary text that includes the stock price if a ticker is found. |
| `setSuperAdmin` (Cloud Function)        | A secure, server-side function to grant or revoke SuperAdmin privileges. Triggered from the SuperAdmin dashboard.     | User email and a boolean `grant` flag.                   | A success or error message.                                          |
| `sendMessage` (Cloud Function)          | A secure, server-side function to handle all chat message submissions. Ensures atomic writes to Firestore.           | Property ID, message content, and user role.             | A success or error status.                                           |